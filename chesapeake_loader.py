...
def load_single_image_class_pair(fname:str, patch_size:int = 32) -> [tf.Tensor]:
    ...
    outs = tf.one_hot(outs, nclasses)
    # Cut down to patch_size
    outs = tf.image.crop_to_bounding_box(outs, 0, 0, patch_size, patch_size)
...
def create_single_dataset(base_dir='/home/fagg/datasets/radiant_earth/pa', full_sat=True, patch_size=None,
                          partition='train', fold=0, filt='*', cache_path=None, repeat=False, shuffle=None,
    ...
    if full_sat:
        ...
        data = data.map(lambda x: tf.py_function(func=load_single_image_class_pair, inp=[x, patch_size], Tout=(tf.float32, tf.float32)), 
                        num_parallel_calls=num_parallel_calls)
...
def create_datasets(base_dir='/home/fagg/datasets/radiant_earth/pa', full_sat=True, patch_size=None,
                    fold=0, train_filt='*[012345678]',
                    cache_dir=None, repeat_train=False, shuffle_train=None, batch_size=8, prefetch=2,
...
@tf.autograph.experimental.do_not_convert
def create_diffusion_example(I:tf.float32, L:tf.float32, patch_size:int, alpha:tf.Tensor, nalpha:tf.Tensor):
    '''
    ...
    :param nalpha: Number of elements in the alpha vector
    ...
    # Convert to float
    #t = tf.cast(t, dtype=tf.dtypes.float32)
    ...
    #T = tf.multiply(tf.ones(shape=(patch_size,patch_size,1)), tf.divide(t, tf.cast(nalpha, dtype=tf.dtypes.float32)))

    return tf.cast(L, tf.dtypes.float32), tf.cast(t, tf.dtypes.int32), tf.cast(image, tf.dtypes.float32), tf.cast(noise, tf.dtypes.float32)

    
def create_diffusion_dataset(alpha, base_dir='/home/fagg/datasets/radiant_earth/pa', 
                            patch_size=None,
                            fold=0, filt='*[012345678]',
                            cache_dir=None, repeat=False, shuffle=None, batch_size=8, prefetch=2,
                            num_parallel_calls=4):
    ...
    :param filt: Regular expression filter for the files within the fold directory. 
                    ...
                    You should not need to change from default unless you want to use a smaller training set
    :param cache_dir: None -> no cache; directory path -> cache to 3 files (on the supercomputer, this is LSCRATCH)
    :param repeat: repeat data set indefinitely (default = False) (validation and test sets are not repeated)
    :param shuffle: shuffle buffer size (default = None -> dont shuffle) (validation and test sets are not shuffled)
    :param batch_size: Size of the batches to be produced by this data set 
    :param prefetch: Number of batches to prefetch in parallel with training 
    :param num_parallel_calls: Number of threads to use for the data loading process (-1 = use AutoTune)
    
    :return: TF Dataset for training formatted for using with model.fit()
        2-Tuple of:
    ...
    ds = ds.map(lambda I, L: tf.py_function(func=create_diffusion_example, inp=[I, L, patch_size, alpha_tf, nalpha], 
    ...
    ds_valid = ds_valid.map(lambda I, L: tf.py_function(func=create_diffusion_example, inp=[I, L, patch_size, alpha_tf, nalpha], 
                                            Tout=(tf.float32, tf.int32, tf.float32, tf.float32)), 
                        num_parallel_calls=num_parallel_calls)
